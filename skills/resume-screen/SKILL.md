...
# /resume-screen — Resume vs Job Screen
...
- **STRONG fit** → "Run `/score {name-slug}` to get the full 9-dimension weighted score before submitting."
- **MODERATE fit** → "Run `/score {name-slug}` to identify which dimensions are dragging the score, then decide if coaching closes the gap."
- **WEAK fit** → "Consider running `/market-map {role} in {location}` to find better-matched candidates in this market."
- **NO FIT** → "This candidate doesn't fit this role. Run `/market-map` to map who does fit, or check other open roles."
