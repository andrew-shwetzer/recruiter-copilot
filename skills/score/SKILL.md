...
# /score — Candidate-Job Fit Scorer
...
- **SUBMIT or SUBMIT WITH COACHING:** "Ready to draft outreach to the client? Run `/outreach` to write the submission message, or `/candidate-msg` to draft the candidate prep message."
- **HOLD:** "The score is borderline. Run `/resume-screen {candidate-slug} against {JD}` for a full evidence audit to identify whether coaching would move the needle."
- **PASS:** "This candidate doesn't fit this role. Run `/market-map {role} in {location}` to map where better-fit candidates are concentrated."
