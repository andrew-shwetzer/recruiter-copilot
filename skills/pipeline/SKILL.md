...
- `/pipeline` — show full pipeline view
- `/pipeline status` — same as above (explicit)
- `/pipeline update <item name> <new-status>` — update a lead or candidate's status

Examples:
- `/pipeline update "Acme Corp" replied`
- `/pipeline update "Jane Smith" submitted`
- `/pipeline update "Stripe" contacted`
...
[Generate 2–4 specific, actionable suggestions based on what's in the pipeline. Examples:]
  • [N] leads have no outreach yet — run /outreach to draft messages
  • [Company name] replied [N] days ago with no follow-up — time to respond
  • [Candidate name] is screened but no outreach sent — run /candidate-msg
  • Pipeline is empty — run /signals to find new leads
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Rules for suggestions:**
- If there are leads with status "new" and count >= 3: "You have [N] leads with no outreach yet — try /outreach [company]"
- If there are leads with status "researched": "Research done on [company] — ready for outreach with /outreach [company]"
- If any leads are STALE: "Follow up with [company] — no movement in [N] days"
- If candidate pipeline is empty but leads exist: "No candidates tracked yet — use /source or /resume-screen to evaluate candidates"
- If pipeline is completely empty: "Pipeline is empty. Start with /signals to find target companies."
- Always show at least one suggestion. Never show more than four.

## Step 6 — Status Update Mode

If the user ran `/pipeline update <item> <new-status>`:
...
Say: "Couldn't find '[item name]' in leads or candidates. Check the spelling or run `/pipeline` to see all tracked items."
